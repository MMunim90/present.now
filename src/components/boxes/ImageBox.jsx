...
export default function ImageBox({ box, ...props }) {
  ...
  useEffect(() => {
    ...
    mediaStorage.getImage(box.metadata.imageId).then((blob) => { if (blob) { objectUrl = URL.createObjectURL(blob); setPreview(objectUrl) } else setError('Image file is unavailable.') }).catch(() => setError('Could not load this image.'))
  ...
  const handleUpload = async (event) => {
    ...
    try { setError(''); await props.onUpload(file) } catch { setError('Could not save this image locally.') }
  }
