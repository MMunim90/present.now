...
export default function ExpandedBox({ box, onChange, onClose }) {
  ...
  useEffect(() => {
    ...
    if (box.type !== 'image' || !box.metadata.imageId) { setImageUrl(null); return undefined }
    mediaStorage.getImage(box.metadata.imageId).then((blob) => { if (!cancelled && blob) { objectUrl = URL.createObjectURL(blob); setImageUrl(objectUrl) } })
  ...
  return <div className="expanded-backdrop" role="presentation" onPointerDown={onClose}><section className="expanded-dialog" role="dialog" aria-modal="true" aria-labelledby="expanded-box-title" onPointerDown={(event) => event.stopPropagation()}><header><div><span>Focused editor</span><h2 id="expanded-box-title">{title}</h2></div><button type="button" className="expanded-close" onClick={onClose} title="Minimize box" aria-label="Minimize box"><Minimize2 size={18} /><span>Minimize</span></button></header><div className="expanded-body">{body}</div><button className="expanded-x" type="button" onClick={onClose} aria-label="Close expanded editor" title="Close"><X size={18} /></button></section></div>
}
